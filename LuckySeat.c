...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, PSTR szCmdLine, int iCmdShow)
{
    ...
    hwnd = CreateWindow(TEXT("LuckySeat"), title, WS_OVERLAPPEDWINDOW, 0, 0, 1024, 690, NULL, NULL, hInstance, NULL);
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    {
    case WM_CREATE:
        hInBox = CreateWindow("EDIT", NULL, WS_CHILD | WS_VISIBLE | ES_LEFT | WS_BORDER | ES_AUTOHSCROLL, 260, 180, 200,
                              36, hwnd, (HMENU)ID_InBox, hInst, NULL);

        hLuckyButton = CreateWindow("BUTTON", TEXT("开始摇号"), WS_CHILD | WS_VISIBLE, 640, 180, 128, 40, hwnd,
                                    (HMENU)ID_LuckyButton, hInst, NULL);

        hViewButton = CreateWindow("BUTTON", TEXT("查看结果"), WS_CHILD | WS_VISIBLE, 820, 180, 128, 40, hwnd,
                                   (HMENU)ID_ViewButton, hInst, NULL);

        hOutBox = CreateWindow("EDIT", NULL,
                               WS_CHILD | WS_VISIBLE | ES_LEFT | WS_BORDER | ES_MULTILINE | WS_VSCROLL | WS_HSCROLL |
                                   ES_AUTOVSCROLL | ES_AUTOHSCROLL,
                               20, 250, 960, 380, hwnd, (HMENU)ID_OutBox, hInst, NULL);
    ...
    case WM_PAINT:
        ...
        hFont = CreateFont(32, 16, 0, 0, FW_REGULAR, FALSE, FALSE, FALSE, GB2312_CHARSET, OUT_DEFAULT_PRECIS,
                           CLIP_DEFAULT_PRECIS, PROOF_QUALITY, FIXED_PITCH | FF_MODERN, "宋体");
    ...
    case WM_COMMAND:
        ...
        case ID_LuckyButton:
            ...
            if ((stu_qty <= 0) || (stu_qty > 10000) || (strlen(itoa(stu_qty, temp, 10)) != strlen(str_input)))
            {

                Beep(1200, 100);
                MessageBox(NULL, "请输入10000以内的正整数 !", "温馨提示!", MB_OK);
                break;
            }

            _ftime(&timeBuf);
            seed = ((((unsigned int)timeBuf.time & 0xFFFF) + (unsigned int)timeBuf.millitm) ^
            ...
            for (i = 0; i < stu_qty; ++i)
            {
                str_len += sprintf(buffer + str_len, "(学%d->座%lu), ", (i + 1), *(result + i));
                ++count;
                if (count % 10 == 0)
            ...
            if (strlen(buffer))
            {
                ...
                if (fp == NULL)
                {
                    MessageBox(NULL, "未能把摇号结果保存到文件!", "温馨提示!", MB_OK);
                    return 1;
                }
                fputs(buffer, fp);
                fclose(fp);
